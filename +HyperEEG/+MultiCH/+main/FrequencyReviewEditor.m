function [reviewResult, cancelbool, excludeFileBool, rerunICABool] = ...
        FrequencyReviewEditor(EEGdata, currentfilename, allowRerunICA)
%FREQUENCYREVIEWEDITOR 最终人工复核的通道×频率Hz交互界面。
%   可标记整条坏导、返回上一步重新ICA、确认整文件无效或确认通过；
%   不收集时间坏段。重新ICA时放弃当前尚未确认的频域标记。
    ...
    confirmControl = uicontrol(fig, 'Style', 'pushbutton', ...
        'Units', 'normalized', 'Position', [0.82 0.255 0.15 0.055], ...
        'String', '确认频域复核', 'Enable', 'off', ...
        'Callback', @confirmReview);
    rerunICAControl = uicontrol(fig, 'Style', 'pushbutton', ...
        'Tag', 'RerunICAFromFrequencyReview', ...
        'Units', 'normalized', 'Position', [0.82 0.185 0.15 0.055], ...
        'String', '退回上一步，重新ICA', ...
        'Callback', @requestRerunICA);
    uicontrol(fig, 'Style', 'pushbutton', ...
        'Units', 'normalized', 'Position', [0.82 0.115 0.15 0.055], ...
        'String', '整文件无效', 'Callback', @excludeFile);
    uicontrol(fig, 'Style', 'pushbutton', ...
        'Units', 'normalized', 'Position', [0.82 0.045 0.15 0.055], ...
        'String', '取消当前文件', 'Callback', @cancelReview);

    if ~allowRerunICA
        rerunICAControl.Enable = 'off';
        rerunICAControl.TooltipString = '人工ICA未开启，无法返回并重新执行ICA';
    end
    ...
    function requestRerunICA(~, ~)
        answer = questdlg( ...
            ['将放弃本轮尚未确认的坏导和可疑频段标记，', ...
            '返回上一步重新进行ICA。是否继续？'], ...
            '重新进行ICA', '重新ICA', '继续复核', '继续复核');

        if strcmp(answer, '重新ICA')
            rerunICABool = 1;
            finishReview(0);
        end
    end
